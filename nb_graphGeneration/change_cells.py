...
from shared.paths import GLOBAL_GRAPH_RESULTS, METADATA_DIR
...
if __name__ == "__main__":
    for time in ['I-II_leidenwu', 'III-IV_leidenwu']:
        ...
        group_path = str(METADATA_DIR / f'groups_{time}')
        net_path = str(GLOBAL_GRAPH_RESULTS / f'net_{time}')
        matrix_path = str(GLOBAL_GRAPH_RESULTS / f'matrix_{time}')

        # Ensure files exist before calling rename_cells to avoid crashes
        net_r = Path(f'{net_path}.txt').exists()
        group_r = Path(f'{group_path}.csv').exists()
        matrix_r = Path(f'{matrix_path}.txt').exists()

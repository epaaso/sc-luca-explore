...
pre-computed sample-by-type matrices stored under `results/graph_generation/global/` and running
ARACNe-AP for every community assignment present in the
`results/ecotype_clustering/membership_by_cluster_*.csv` files. Each cluster run
receives its own subfolder inside `results/graph_generation/cluster_runs/<stage>/` with the
...
Example usage (dry run for cluster 0 early stage):
    ...
    python nb_graphGeneration/run_aracne_by_cluster.py --stages early --clusters 0 --dry-run

Run all stages and clusters with 200 bootstraps and 2 GiB of JVM memory:

    python nb_graphGeneration/run_aracne_by_cluster.py --bootstraps 100 --java-memory 1G
...
from shared.paths import (
    CLUSTER_GRAPH_RESULTS,
    EARLY_MEMBERSHIP,
    GLOBAL_GRAPH_RESULTS,
    LATE2_MEMBERSHIP,
    LATE_MEMBERSHIP,
)
...
DEFAULT_OUTPUT_ROOT = CLUSTER_GRAPH_RESULTS
STAGE_CONFIGS = {
    "early": StageConfig(
        name="early",
        membership_csv=EARLY_MEMBERSHIP,
        matrix_path=GLOBAL_GRAPH_RESULTS / "matrixI-II_leidenwu_funcnames.txt",
        time_label="I-II_leidenwu",
    ),
    "late": StageConfig(
        name="late",
        membership_csv=LATE_MEMBERSHIP,
        matrix_path=GLOBAL_GRAPH_RESULTS / "matrixIII-IV_leidenwu_funcnames.txt",
        time_label="III-IV_leidenwu",
    ),
    "late2": StageConfig(
        name="late",
        membership_csv=LATE2_MEMBERSHIP,
        matrix_path=GLOBAL_GRAPH_RESULTS / "matrixIII-IV_leidenwu_funcnames.txt",
        time_label="III-IV_leidenwu",
    ),
}
...
def process_stage(stage: StageConfig, args: argparse.Namespace) -> None:
    ...
    stage_output_dir = (
        args.output_root / stage.name
        if args.dry_run
        else generate_stage_output_dir(args.output_root, stage)
    )
    if not args.dry_run:
    ...
    for cluster_id in cluster_ids:
        ...
        tf_list = aracne_matrix.index.tolist()
        logging.info(
        ...
        if args.dry_run:
            logging.info(
                "Dry-run enabled; skipping input writes and ARACNe execution for cluster %d",
                cluster_id,
            )
            continue

        matrix_path, tf_path, _ = write_matrix_and_tfs(
